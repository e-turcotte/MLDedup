...
print(f"NDCG@1         = {heuristic['mean_ndcg1']:.4f}  "
      f"({sum(heuristic['ndcg1_scores']):.0f}/{len(heuristic['ndcg1_scores'])} groups picked the best module)")
...
print("\n--- ML model (in-sample) ---")
print(f"NDCG@1         = {model['mean_ndcg1']:.4f}  "
      f"({sum(model['ndcg1_scores']):.0f}/{len(model['ndcg1_scores'])} groups picked the best module)")
...
if cv is not None:
    ...
    print(f"NDCG@1         = {cv['mean_ndcg1']:.4f}  "
          f"({sum(cv['ndcg1_scores']):.0f}/{len(cv['ndcg1_scores'])} groups picked the best module)")
...
print(f"NDCG@1 gap (ML - heuristic) = {model['mean_ndcg1'] - heuristic['mean_ndcg1']:+.4f}")
print(f"Tau gap    (ML - heuristic) = {model['mean_tau'] - heuristic['mean_tau']:+.4f}")

if cv is not None:
    print(f"\n--- Comparison (leave-one-design-out CV) ---")
    print(f"NDCG@1 gap (ML - heuristic) = {cv['mean_ndcg1'] - heuristic['mean_ndcg1']:+.4f}")
